...
statically_link_dependencies_with_libvcx(){
    echo "${BLUE}Statically linking libraries togather${RESET}"
    echo "${BLUE}Output will be available at ${ANDROID_BUILD_FOLDER}/libvcx_${ABSOLUTE_ARCH}/lib/libvcx.so${RESET}"
    $CC -v -shared -o${ANDROID_BUILD_FOLDER}/libvcx_${ABSOLUTE_ARCH}/lib/libvcx.so -Wl,--whole-archive \
        ${WORKDIR}/target/${TRIPLET}/release/libvcx.a \
...
package_library(){
   ...
   export PACKAGE_DIR=${ANDROID_BUILD_FOLDER}/libvcx_${ABSOLUTE_ARCH}
    ...
    if [ "${TARGET_ARCH}" != "x86_64" ]; then
        mv "${PACKAGE_DIR}/lib/libvcx.so" "${PACKAGE_DIR}/lib/libvcx_shared.so" &&
        statically_link_dependencies_with_libvcx
    fi
    pushd ${LIBVCX_WORKDIR}
        ...
        if [ -z "${LIBVCX_VERSION}" ]; then
            ...
            echo "${BLUE}Zip file available at ${PWD}/libvcx_android_${ABSOLUTE_ARCH}.zip ${RESET}"
        else
            zip -r libvcx_android_${ABSOLUTE_ARCH}_${LIBVCX_VERSION}.zip libvcx_${ABSOLUTE_ARCH} &&
            echo "${BLUE}Zip file available at ${PWD}/libvcx_android_${ABSOLUTE_ARCH}_${LIBVCX_VERSION}.zip ${RESET}"
        fi
...
build(){
    ...
    echo "Artifacts will be in ${YELLOW}${GREEN}${ANDROID_BUILD_FOLDER}/libvcx_${ABSOLUTE_ARCH}${RESET}"
    echo "**************************************************"
    pushd ${WORKDIR}
